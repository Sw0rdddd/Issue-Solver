...
- 不自动创建虚拟环境，不安装依赖；环境由目标仓库开发者提前准备。
...
项目检测当前主要识别 Python 项目标记，并发现基础 pytest 命令（例如 `pytest -q`）。如果没有可支持的测试入口，工作流不会假设其他测试框架可执行。
...
Review 后仍会执行真实测试，而不是只依赖 Review 结论。Test 节点将检测到的逻辑命令解析为 argv，最终统一绑定为：
...
执行器不允许管道、重定向等 Shell 控制字符，只支持 `pytest ...` 或 `python/py -m pytest ...` 形式，并以 `shell=False` 启动。每条测试会落盘完整 stdout/stderr；给 Coordinator 的仅是受行数和 20,000 字符限制的末尾摘要。测试前后还会计算工作区指纹，若测试本身修改了 Git 工作区，会标记为环境错误并走保护路径。
...
| 基线与环境 | `base_commit`、`project_type`、`test_commands`、`environment` | 固化可重复验证的仓库和运行环境 |
...
| `CodingTask` | `objective`、`acceptance_criteria`、`root_cause`、`relevant_files`、`allowed_scope`、`validation` | 将“改一下”变成带允许路径边界的可验证任务 |
...
| `test_stdout_r01_i01.log`、`test_stderr_r01_i01.log` | 第 1 条测试命令的完整进程输出 |
