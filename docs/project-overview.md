...
6. 在当前 10 个真实 GitHub Issue 的基础上继续扩展仓库、模型和重复运行次数，增加稳定性、方差、越界修改率和失败可恢复率等指标。
...
项目当前评测集包含 10 个真实开源项目 GitHub Issue。每个案例固定修复前基线，并附带能在基线失败、修复后通过的回归测试；Issue 当前可能已经修复、关闭、重新打开或内容变化，评测结果不代表 GitHub 上的当前状态。

2026-07-22 至 2026-07-23 使用 `deepseek-v4-pro` 完成的端到端结果为：

| Issue 数 | 成功修复 | 成功率 | Patch 生成率 | 测试通过率 | 平均耗时 | 平均轮次 | 平均 Token |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 10 | 9 | 90.00% | 90.00% | 90.00% | 405.66 秒 | 1.20 | 761,799.50 |

测试通过率按全部 Issue 计算，只有定向测试和全量回归都通过才计为通过。本轮采用推理模型，405.66 秒为包含模型推理、工具调查、代码修改、Review 与真实 pytest 的端到端耗时，并非单次模型生成延迟。缓存读取为 6,663,040 Token，占输入 Token 的 90.75%；该比例表示缓存读取 Token 占输入 Token，不是按请求数量计算的命中率。唯一失败发生在 Explore 的结构化输出阶段，未生成最终 Patch，也未进入测试。
