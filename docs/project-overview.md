...
一个 CodingTask 可以通过一次 Patch 同时修改多个相关文件，也可以在同一 Coding 阶段最多进行 10 次 `apply_patch` 尝试；但整个 Coding 节点是串行的。第 10 次失败后节点会终止并回滚；超过限制的 Patch 会记录但不会应用。这样既允许“代码 + 测试”作为原子任务一起改，也避免了多个 Agent 争抢同一工作区。
...
Review Agent 是只读角色，能读取当前工作区、搜索代码并查看相对基线的 Git Diff。它输出 `ReviewResult`：`APPROVE` 时 `issues` 必须为空；`REQUEST_CHANGES` 时必须给出至少一个具体问题。结构校验或 Agent 失败会保存 `logs/failure_review_rXX.json`，保留工作区，并在 CLI 交互场景询问用户是否回滚。
...
<目标虚拟环境的 python> -m pytest <原测试参数>
    --basetemp=<运行目录>/logs/<测试临时目录>
    -o cache_dir=<运行目录>/logs/<测试缓存目录>
```

测试进程的 `cwd` 是目标仓库根目录，但 `TEMP`、`TMP`、`TMPDIR`、pytest basetemp 和 pytest cache 都指向运行目录的 `logs/`；同时通过 `VIRTUAL_ENV` 或 `CONDA_PREFIX` 与 PATH 前缀明确使用目标环境。这样既满足“必须 `python -m pytest`”的解释器一致性，也避免系统临时目录或业务仓库中的 pytest 缓存权限/污染问题。
...
LangGraph 结束且交互式回滚决策完成后，CLI 执行独立的 Report 收尾。Reporter 不绑定任何工具，只接收程序筛选后的 Issue、探索结论、真实修改文件、Review、最新测试结果和最终工作区状态，并输出简洁中文 Markdown。完整测试日志、解释器命令和 Diff 不会进入 Reporter 上下文。

报告以 UTF-8 保存为运行目录根部的 `report.md`。环境预检失败、模型调用失败或 Reporter 返回空文本时，程序使用同一批状态生成固定模板；报告生成失败不会改变原工作流的退出状态。
...
| Reporter | 否 | 无 | 最终状态、回滚结果、修改/审查/测试摘要 | `report.md` Markdown |
...
每一次运行都拥有一个 ULID 风格 `run_id` 和独立目录。详细 JSON、audit、测试输出和运行时目录统一写入 `run_id/logs/`；面向开发者的 `report.md` 和最终 `diff.patch`、`diff.json` 保留在 `run_id/` 根部。所有 JSON 以 UTF-8 写入，并使用排他创建（`x` 模式）避免悄悄覆盖同名证据；最终 Patch 使用原子写入。

常见产物如下：

| 文件模式 | 内容 |
| --- | --- |
| `logs/environment_result.json` / `logs/failure_environment.json` | 环境预检结果或失败原因；失败文件还记录未调用 LLM、未安装依赖、未修改工作区 |
...
| `logs/coding_audit_r01_s01.jsonl` | 每次 `apply_patch` 的原始与规范化 Patch、规范化动作、成功/失败、触碰文件和 Diff 哈希；一行一个事件 |
...
| `logs/rollback_decision_r01.json` | Review 失败后用户是否选择回滚 |
| `report.md` | 面向开发者的最终运行摘要；模型不可用时由程序模板生成 |
