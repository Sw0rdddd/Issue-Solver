...
`parse_issue` 节点先由 `src/services/issue_loader.py` 读取原始输入，再让模型输出 `IssueSpec`。它将不稳定的自然语言统一成标题、原始描述、期望行为、实际行为和验收条件，后续角色不必反复从原文猜测任务。验收条件遵循原文优先和最小推导：原文已经明确时保留原意，没有显式清单但期望可以直接推出时生成最少的可验证条件；存在多种合理解释时终止并要求补充输入。
