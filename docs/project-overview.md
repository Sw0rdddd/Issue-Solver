...
- `MAX_CYCLES` 默认是 5，达到限制会记录失败原因；已有修改时由统一终态机制询问是否回滚。
...
| 工作区与失败 | `changed_files`、`diff_path`、`rollback_required`、`failure`、`rollback_failure` | 控制 Patch 保存、回滚和结构化错误展示 |
...
| `CoordinatorDecision` | `next_action`、`current_summary`、`explore_focuses`、`coding_task`、`failure` | 用互斥校验限制 Coordinator 的路由输出；只有 `FAILED` 必须携带 failure |
...
这是一次直接接口升级：新运行产物使用 `failure`、`explore_failures` 和 `rollback_failure`，不再写入旧的 `error`、`explore_errors`、`rollback_reason` 或 `rollback_error` 字段；历史运行目录不会自动迁移。
